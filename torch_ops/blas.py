...
from _wrappers import register_bwd_op
...
def addbmm_bwd(dl_dout, batch1, batch2, beta, alpha, *, out_mask=[True, True, True], out=[None, None, None]):
    ...
    if out_mask[1]:
        dl_dbatch1 = torch.mul(torch.vmap(torch.mm, in_dims=(None, 0), out_dims=0)(dl_dout, batch2.t(), out=out[1]), alpha, out=out[1])
    else:
        dl_dbatch1 = None
    
    if out_mask[2]:
        dl_dbatch2 = torch.mul(torch.vmap(torch.mm, in_dims=(0, None), out_dims=1)(batch1.t(), dl_dout, out=out[2]), alpha, out=out[2])
    else:
        dl_dbatch2 = None
...
def addmm_bwd(dl_dout, mat1, mat2, beta, alpha, *, out_mask=[True, True, True], out=[None, None, None]):
    ...
    if out_mask[1]:
        dl_dmat1 = torch.mul(torch.mm(dl_dout, mat2.t(), out=out[1]), alpha, out=out[1])
    else:
        dl_dmat1 = None

    if out_mask[2]:
        dl_dmat2 = torch.mul(torch.mm(mat1.t(), dl_dout, out=out[2]), alpha, out=out[2])
    else:
        dl_dmat2 = None
...
def addmv_bwd(dl_dout, mat, vec, beta, alpha, *, out_mask=[True, True, True], out=[None, None, None]):
    ...
    if out_mask[2]:
        dl_dvec = torch.mul(torch.mv(mat.t(), dl_dout.unsqeeuze(1)), alpha, out=out[2]).squeeze(1)
    else:
        dl_dvec = None
...
def baddbmm_bwd(dl_dout, batch1, batch2, beta, alpha, *, out_mask=[True, True, True], out=[None, None, None]):
    ...
    if out_mask[1]:
        dl_dbatch1 = torch.mul(torch.bmm(dl_dout, batch2.t(), out=out[1]), alpha, out=out[1])
    else:
        dl_dbatch1 = None

    if out_mask[2]:
        dl_dbatch2 = torch.mul(torch.bmm(batch1.t(), dl_dout, out=out[2]), alpha, out=out[2])
    else:
        dl_dbatch2 = None
...
def bmm_bwd(dl_dout, input, mat2, *, out_mask=[True, True], out=[None, None]):
    if out_mask[0]:
        dl_dinput = torch.bmm(dl_dout, mat2.t(), out=out[0])
    else:
        dl_dinput = None

    if out_mask[1]:
        dl_dmat2 = torch.bmm(input.t(), dl_dout, out=out[1])
    else:
        dl_dmat2 = None
...
def chain_matmul_bwd(dl_douts, *matrices, mask_out=None, out=None):
    ...
    for i in range(len(dl_douts)):
        if not mask_out[i]:
            ...
            continue
        if i == 0:
            dl_dins.append(torch.matmul(dl_douts[i], torch.chain_matmul(dl_douts[i+1:]).t(), out=out[i]))
        elif i == len(dl_douts)-1:
            dl_dins.append(torch.matmul(torch.chain_matmul(dl_douts[:i]).t(), dl_douts[i], out=out[i]))
        else:
            dl_din = torch.matmul(torch.matmul(torch.chain_matmul(dl_douts[:i]).t(), dl_douts[i]), torch.chain_matmul(dl_douts[i+1:]).t(), out=out[i])
...
def inverse_bwd(dl_dout, inv_t, *, out=None):
    return torch.chain_matmul(inv_t(), dl_dout, inv_t, out=out, out=out)
...
def det_bwd(dl_dout, A, det_A, *, out=None):
    return torch.mul(torch.inv(A).t(), dl_dout*det_A, out=out)
...
def matmul_bwd(dl_dout, input, mat2, *, out_mask=[True, True], out=[None, None]):
    if out_mask[0]:
        dl_dinput = torch.matmul(dl_dout, mat2.t(), out=out[0])
    else:
        dl_dinput = None

    if out_mask[1]:
        dl_dmat2 = torch.matmul(input.t(), dl_dout, out=out[1])
    else:
        dl_dmat2 = None
    return dl_dinput, dl_dmat2
...
def matmul_bwd(dl_dout, input, other, *, out_mask=[True, True], out=[None, None]):
    ...
    if out_mask[0]:
        dl_dinput = torch.matmul(dl_dout, other.t(), out=out[0])
    else:
        dl_dinput = None
    
    if out_mask[1]:
        dl_dother = torch.matmul(input.t(), dl_dout, out=out[1])
    else:
        dl_dother = None
    return dl_dinput, dl_dother
...
def matrix_power_bwd(dl_dout, input, n, *, out=None):
    ...
    for i in range(n):
        if i == 0:
            dl_din += torch.matmul(dl_dout, torch.matrix_power(input, n-1).t())
        elif i == n-1:
            dl_din += torch.matmul(torch.matrix_power(input, n-1).t(), dl_dout)
        else:
            dl_din += torch.matmul(torch.matmul(torch.matrix_power(input, i).t(), dl_dout), torch.matrix_power(input, n-1-i).t()) # not 100% sure about this
...
def mv_bwd(dl_dout, mat, vec, *, out_mask=[True, True], out=[None, None]):
    ...
    if out_mask[1]:
        dl_dvec = torch.mv(mat.t(), dl_dout, out=out[1])
    else:
        dl_dvec = None
